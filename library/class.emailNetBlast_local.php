...
class emailNetBlast{
	...
	public function addEmailBlast($record)
	{
		...
		$privacyContent = str_replace('{RegisterLink}', $registerNow, $tmpPrivacyContent);
		
		$privacyContent .= '<table border="0" align="center" cellpadding="0" cellspacing="0" style="max-width:620px;width:100%;color:#666666;font-size: 10pt;font-family: Arial,sans-serif;">
							  <tr>
        ...
        if($record['view_online']==1){
                $browserView = '<p style="font-size:10px; text-align:center; font-family:Arial,Verdana;">Having trouble viewing this email? <a href="https://love.officeinteractive.com/onlineView.php?viewerId=[clientId]&viewerIs={LeadID}&contentIs='.base64_encode($record['blast_id']).'&viewerTitle='.base64_encode($record['blast_subject']).'">View online</a></p>';
        }else{
		...
		if($record['event_id']){
			$templateFinal = htmlentities(mb_convert_encoding($privacyContent,"UTF-8"));
		} else {
			if(strtolower(strpos($message, "</body>") !== false)){
				$templateFinalBdy = preg_replace("</body>", $privacyContent, $message);
                $templateFinal =  $browserView.''.$templateFinalBdy;
			} else {
				$templateFinal = $browserView.''.$message.''.$privacyContent;
			}
		...
		if($record['disable_background']==1){
			...
			<table bgcolor="#fff" width="620" style="padding: 13px 12px; background: #fff; border: 2px solid rgba(0,0,0,0.1); " align="center" cellpadding="0" cellspacing="0" border="0">
			<tr><td><![endif]-->';
			$outLookStyleEnd ='<!--[if (gte mso 9)|(IE)]></td></tr></table><![endif]-->';
			$templateFinal = '<html><head><style> @media screen and (max-width:525px) { div { width:100% !important;  } table{ width:100% !important;padding-left: 0px !important;padding-right: 0px !important; } td { width:100% !important;display:block !important;padding-left: 0px !important;padding-right: 0px !important;margin:0px !important;  }  img {  max-width:100% !important; } } '.$blastCustomCss.' </style></head>
			<table bgcolor="#fff" border="0" align="center" class="removeTable" style=" padding: 13px 12px; max-width:620px;margin:0 auto;background: #fff; border: 0px; "><tr class="removeTable"><td width="100%" class="removeTable" style="width:100%;">
			...
			<table bgcolor="#fff" border="0" align="center" class="removeTable" style=" padding: 13px 12px; max-width:620px;margin:0 auto;background: #fff; border: 2px solid rgba(0,0,0,0.1); "><tr class="removeTable"><td width="100%" class="removeTable" style="width:100%;">
			'.$outLookStyle. $templateFinal . $outLookStyleEnd. '
			</td></tr></table>
