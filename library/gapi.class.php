...
class gapi
{
   ...
   * @param Int $max_results OPTIONAL: Max results returned
   */
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $segment=null, $start_date=null, $end_date=null, $start_index=1, $max_results=10000)
  {
  ...
  */
  ...
  protected function reportObjectMapper($xml_string)
  {
	...
	$report_root_parameters['tableId'] = (string) $google_results->dataSource->tableId;
	$report_root_parameters['tableName'] = (string) $google_results->dataSource->tableName;
  ...
  private function fopenRequest($url, $get_variables=null, $post_variables=null, $headers=null)
  {
	...
	echo '--------- URL: '. $url.$get_variables;
    print_r($http_options);
    $context = stream_context_create(array('http'=>$http_options));
	echo "\n-----------------------------\n";
    echo $response = file_get_contents($url . $get_variables, false, $context);  
	echo "\n-----------------------------\n";
...
class gapiReportEntry
{
  ...
  /**
   * Function to covert the given dimensions & metrics in csv format
   * 
   * @params $seperator Seperator used to join the different fields
   * @return String
   */
  
  public function toCSV($seperator='#')
  {
  	$dimensions = '';
	if(is_array($this->dimensions))
    {
      $dimensions =  implode($seperator, $this->dimensions);
    }
	$metrics = implode($seperator, $this->metrics);
	return $dimensions.$seperator.$metrics;
  }
  /**
