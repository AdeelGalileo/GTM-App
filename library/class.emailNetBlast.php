...
class emailNetBlast{
	...
	public function addEmailBlast($record)
	{
        ...
        if($record['view_online']==1){
				...
				$browserView = '<p style="font-size:7px;line-height:1; text-align:center; font-family:Arial,Verdana;">
				<a href="javascript:void(0);" style="display:none;"><img src="https://love.officeinteractive.com/images/placeholder.png?viewerId=[clientId]&viewerIs={LeadID}" style="display:none;"></a>
				</p>';
		}
		...
		if($record['blast_is_new']==0){
			$paddingForOldEditor="padding:10px;";
		}
		else{
			$paddingForOldEditor="";
		}
		if($record['disable_background']==1){
			$backgroundDisableStyle='<!--[if (gte mso 9)|(IE)]>
			...
			$border_style="border: 2px solid #fff;";
		}
		else{
			$backgroundDisableStyle='<!--[if (gte mso 9)|(IE)]>
			...
			<tr><td><![endif]-->';
			$bgcolor="#eee";
			$background_style_color="background-color:#eee;";
			$border_style="border: 2px solid rgba(0,0,0,0.1);";
		}
			
		$outLookStyle = '<!--[if (gte mso 9)|(IE)]>'
		.$backgroundDisableStyle;
		$outLookStyleEnd ='<!--[if (gte mso 9)|(IE)]></td></tr></table><![endif]-->';
		$templateFinal = '<html><head><style> @media screen and (max-width:525px) { div { width:100% !important;  } table{ width:100% !important;margin-left:0px !important;margin-right:0px !important; }   img {  max-width:100% !important; } }  img { padding:0px !important;margin:0px; } .social img{ display: inline-block; } p{ padding:0px !important;margin:0 0 10px 0px !important; clear:both !important; } h1,h2,h3,h4,h5,h6{ clear:both !important; } table { mso-table-lspace:-1pt; mso-table-rspace:-1pt; } td, p , a, h1, h2, h3, h4, h5, h6 { font-family: Century Gothic, sans-serif; }  td { font-size:16px; } '.$blastCustomCss.' </style></head>
		<table align="center" border="0" cellspacing="0" cellpadding="0" width="100%" bgcolor="'.$bgcolor.'" style="table-layout:fixed;width:100%;'.$background_style_color.'"> <tbody> <tr> <td align="center"> <center style="width:100%;">
		'.$browserView.'
		<table bgcolor="#fff" border="0" cellpadding="0" cellspacing="0"  align="center" width="615" class="removeTable" style="max-width:615px;margin:0 auto;background: #fff;'.$border_style.$paddingForOldEditor.'"><tr class="removeTable"><td width="100%" class="removeTable" style="width:100%;">
