<?php
class qbClassReferenceListModule {
	function display($db = '', $params = array()) {
		...
		$qbClassRefList = $qbClassReferenceObj->getQbClassReferenceList($params);
		
		$page  = ($params['page']) ? $params['page']  : 1;
		$limit = (int) $params['recCount'];
		$start = ($params['page'] - 1) * $limit;
		
		$totalRecords = getTotalRecords($qbClassRefList);
		...
		$smarty->assign('qbClassRefList', $qbClassRefList['qbClassRefList']);
