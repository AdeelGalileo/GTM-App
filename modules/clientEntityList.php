...
class clientEntityListModule {
	function display($db = '', $params = array()) {
		...
		$clientEntityList = $clientEntityObj->getClientEntityList($params);
		
		$page  = ($params['page']) ? $params['page']  : 1;
		$limit = (int) $params['recCount'];
		$start = ($params['page'] - 1) * $limit;
		
		$totalRecords = getTotalRecords($clientEntityList);
		...
		$smarty->assign('clientEntityList', $clientEntityList['clientEntityList']);
