...
class createBillQbModule {
	function display($db = '', $params = array()) {
		...
		$outputArray=array();
		$qbSuccess =false;
		$refBilTypeArr = $params['refBilTypeArr'];
		...
		if($refBilTypeArr){
			...
			try {
				...
				$billArray = $billItemArray=$SalesItemLineDetail=$itemRef=$ItemRefVal=$classRef=$classRefVal=$vendorRef= $attachArray = array();
				
				foreach($refBilTypeArr as $refKey => $refVal){
					...
					$billItemArray['Amount']=$refVal['sum'];
					$billItemArray['DetailType']="AccountBasedExpenseLineDetail";
					$billItemArray['AccountBasedExpenseLineDetail']=array('AccountRef'=>QB_ACCOUNT_REFERENCE_FOR_ALL, 'ClassRef'=>$classRef['ClassRef']);
