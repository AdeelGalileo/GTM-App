...
class skillListModule {
	function display($db = '', $params = array()) {
		...
		$skillsList = $skillsObj->getConsultantSkillList($params);
		
		$page  = ($params['page']) ? $params['page']  : 1;
		$limit = (int) $params['recCount'];
		$start = ($params['page'] - 1) * $limit;
		
		$totalRecords = getTotalRecords($skillsList);
		...
		$smarty->assign('consultantSkillList', $skillsList['consultantSkillList']);
		$smarty->assign('consultantSkillListItems', $skillsList['consultantSkillListItems']);
