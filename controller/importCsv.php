...
class importCsv extends table{
	...
	{	
		...
		foreach($insertRecord as $dataLine => $record){
			...
			$inputArray['noPagesData']=$record['Number of Pages'];
			$inputArray['notesData']=$record['Notes'];
			$inputArray['boxLocationData']=$record['BOX Link'];
			...
			$inputArray['setupDueData']=(!empty($record['Keyword File Set Up Due'])) ? convertDateToYMD($record['Keyword File Set Up Due']) : "";
			$inputArray['dueData']=(!empty($record['Keywords Due'])) ? convertDateToYMD($record['Keywords Due']) : "";
			...
			if($record['MARSHA'] =='' && $record['Number of Pages'] ==''  && $record['Notes'] == '' && $record['BOX Link'] == '' && $record['Date Added to BOX'] == '' && $record['Keyword File Set Up Due'] == '' && $record['Keywords Due'] == ''){
				continue;
			}
