...
<table class="table table-bordered table-striped">
		...
		<tr class="bg-blueL" id="overAllForm">
			...
			<th style="color:#337ab7;">Contact Info</a></th>
			...
			{if $formList}
				{foreach $formList as $key=> $value}
					<tr class="">
						...
						<td>
							{if $value.form_email} {$value.form_email} {/if}
							{if $value.form_contact_no} 
								<br/>
								{$value.form_contact_no} 
							{/if}
							{if $value.form_street} 
								<br/>
								{$value.form_street} 
							{/if}
							{if $value.form_city} 
							...
							{if $value.form_state} 
								...
								{$value.form_state} 
							{/if}
							{if $value.form_zipcode} 
								<br/>
								{$value.form_zipcode} 
							{/if}
							{if $value.form_country} 
								<br/>
								{$value.form_country} 
							{/if}
						</td>
						...
						<td>
						{*<a href="javascript:void(0)" class="formUpdateAction m-l-10" id="formId_{$value.form_id}" title="Update"><i class="fa fa-pencil"></i></a>*}
