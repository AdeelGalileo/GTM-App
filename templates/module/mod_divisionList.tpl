{strip}
<div class="row-fluid">
  <div class="p-t-15 p-b-15"> {if $totalRecords > 0} <span class="blue bold p-t-10 pull-left" style="margin-right:5px; font-size: 15px;"> <span id="totalAutomation">{if $totalRecords==''}0{else} {$totalRecords} {/if}</span> Division</span>
...
<table class="table table-bordered table-striped">
		...
		<tr class="bg-blueL" id="overAllClient">
			<th id="division_codes"><a class="" href="javascript:void(0)" id="division_code">Division Code</a></th>
			<th id="division_created_ons"><a class="" href="javascript:void(0)" id="division_created_on">Created On</a></th>
			...
			{if $divisionList}
				{foreach $divisionList as $key=> $value}
					<tr class="">
						...
						<td>{$value.division_created_on|date_format:$smarty.const.CALENDAR_DATE_FORMAT}</td>
						<td>
							<a href="javascript:void(0)" class="clientUpdateAction m-l-10" id="clientId_{$value.division_id}" title="Update"><i class="fa fa-pencil"></i></a>
							<a href="javascript:void(0)" class="clientDeleteAction m-l-10" id="clientId_{$value.division_id}" title="Delete"><i class="fa fa-trash"></i></a>
						</td>
					</tr>
				{/foreach}
			 {else}
...
{if $totalRecords > 0}
<div class="row-fluid">
  <div class="col-sm-12 P0"> <span class="blue bold p-t-10 pull-left" style="margin-right:5px; font-size: 15px;"> <span id="totalAutomationNew">{if $totalRecords==''}0{else}{$totalRecords}{/if}</span> Division</span>
