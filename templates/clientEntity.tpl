...
<script type="text/javascript">
  ...
  $(document).ready(function(){
	    ...
	    loadClientEntityList(50,orderIdClient,sortIdClient,startDateParam,endDateParam,clientFilterBy,clientIdFilter);
	     ...
	     $('body').on('click', '.descendingClassClient', function(){
			...
			loadClientEntityList(recCountValues,orderIdClient,sortIdClient,startDateParam,endDateParam,clientFilterBy,clientIdFilter);
		});
     ...
     /* --------- Company LIST PAGINATION STARTS HERE ------------------- */
      ...
      $('body').on('change', '#clientPageListing', function(){
          ...
          pageNo = 0;
		  loadClientEntityList(recCount,orderIdClient,sortIdClient,startDateParam,endDateParam,clientFilterBy,clientIdFilter);
      });
      $('body').on('change', '#clientPageListingNew', function(){
	   ...
	   $('#client_entity_client_id').chosen({ width:"100%"});
	   $('#client_entity_division_id').chosen({ width:"100%"});
		...
		$( ".clientFilterBy" ).change(function() {
			...
			if(clientFilterRangeVal.length > 0){
				...
				loadClientEntityList(50,orderIdTask,sortIdTask,startDateParam,endDateParam,clientFilterBy,clientIdFilter);
			}
		});
	   ...
	   $('body').on('click', '.clientUpdateCancel', function(){
		...
		$('body').on('click', '.okClientDelButton', function(){
			...
			$.ajax({
				...
				data : { clientEntityDelete: 1, clientEntityDeleteId:fieldId },
				dataType:'json', 
		...
		$('body').on('click', '.addClientSave', function(){
			...
			var client_entity_division_id = $('.client_entity_division_id').val();
			var client_entity_marsha_code = $('.client_entity_marsha_code').val();
			var client_entity_hotel_name = $('.client_entity_hotel_name').val();
			var client_entity_street = $('.client_entity_street').val();
			var client_entity_city = $('.client_entity_city').val();
			var client_entity_state = $('.client_entity_state').val();
			var client_entity_zipcode = $('.client_entity_zipcode').val();
			var client_entity_country = $('.client_entity_country').val();
			var clientEntityUpdateId = $('.clientEntityUpdateId').val();
			
			
			if(client_entity_division_id==''){
				  showMessage('{$smarty.const.ERR_DIVISION}');
				  $('.client_entity_division_id').focus();
				  return false;
			 }
			 if(client_entity_marsha_code==''){
				  showMessage('{$codeLabelData} is Required');
				  $('.client_entity_marsha_code').focus();
				  return false;
			 }
			 if(client_entity_hotel_name==''){
				  showMessage('{$smarty.const.ERR_LOCATION_NAME}');
				  $('.client_entity_hotel_name').focus();
				  return false;
			 }
		   
		   if(clientEntityUpdateId > 0){
				actionOperand = 2;
			}
			else{
				actionOperand = 1;
			...
			$.ajax({
				...
				type: 'post',
				dataType: 'json',
				data: { actionOperand: actionOperand,  client_entity_division_id: client_entity_division_id, client_entity_marsha_code: client_entity_marsha_code, client_entity_hotel_name: client_entity_hotel_name,  client_entity_street: client_entity_street, client_entity_city: client_entity_city, client_entity_state: client_entity_state, client_entity_zipcode: client_entity_zipcode, client_entity_country: client_entity_country ,clientEntityUpdateId:  clientEntityUpdateId },
				success: function(response) {
					checkResponseRedirect(response);
					showMessage(response.message);
					$(".addClient").hide();
	  ...
	  $('body').on('click', '.clientUpdateAction', function(){
		$(".addClient").hide();
		$(".updateClient").show();
		var clientEntityUpdateId = $(this).attr('id').split('_')[1];
	   
		$.ajax({
			url: 'getModuleInfo.php?rand='+Math.random(),
			type:'post',
			dataType: 'json',
			data : { module: 'clientEntityUpdate', clientEntityUpdateId: clientEntityUpdateId },
				  success: function(response) {
					checkResponseRedirect(response);
					$('.updateClient').html("");
   ...
   function loadClientEntityList(recCountValue,orderId="",sortId="",startDate="",endDate="",clientFilterBy="",clientIdFilter=""){
      ...
      $.ajax({
          ...
          type:'post',
          dataType: 'json',
          data : { module: 'clientEntityList', page: page, recCount: recordCount, orderId: orderId, sortId: sortId, fromDate: startDate, toDate : endDate,clientFilterBy:clientFilterBy, clientIdFilter: clientIdFilter },
          success: function(response) {
              checkResponseRedirect(response);
			  $('#listingTabs').html("");
   		...
   		if(strClassClients && strClassClients!=''){
			  if(strClassClients=='ascendingClassClient'){
				...
				$('#client_entity_marsha_codes a').addClass('descendingClassClient');
				$('#client_entity_hotel_names a').addClass('descendingClassClient');
				$('#client_entity_created_ons a').addClass('descendingClassClient');
			 }
          }
      });	 
   }	
         ...
         <div class="box-body">
			<div class="row">
				...
				<div class="form-group col-sm-3">
					...
					<input type="text" class="form-control client_entity_marsha_code" id="client_entity_marsha_code" placeholder="">
				</div>
				<div class="form-group col-sm-3">
				   <label >Location Name</label>
				   <input type="text" class="form-control client_entity_hotel_name" id="client_entity_hotel_name" placeholder="">
				</div>
				<div class="form-group col-sm-3">
					<label >Street</label>
					<input type="text" class="form-control client_entity_street" id="client_entity_street" placeholder="">
				</div>
			 </div>
			 ...
			 <div class="row">
				...
				<div class="form-group col-sm-3">
				   <label >City</label>
				   <input type="text" class="form-control client_entity_city" id="client_entity_city" placeholder="">
				</div>
				<div class="form-group col-sm-3">
				   <label >State</label>
				   <input type="text" class="form-control client_entity_state" id="client_entity_state" placeholder="">
				</div>
				<div class="form-group col-sm-3">
				   <label >Zip Code</label>
				   <input type="text" class="form-control client_entity_zipcode" id="client_entity_zipcode" placeholder="">
				</div>
				<div class="form-group col-sm-3">
					<label >Country</label>
					<input type="text" class="form-control client_entity_country" id="client_entity_country" placeholder="">
				</div>
			 </div>
   ...
   <div class="col-sm-12">
      <div class="box box-primary">
         ...
         <div class="row ">
            ...
            <div class="col-sm-5">
               <button type="button" class="btn btn-primary pull-right m-r-10 addNewClient"><i class="fa fa-plus"></i> Add New </button>
            </div>
         </div>
         <!-- /.box-header -->
         <div class="box-body">
				...
				<div class="uiContent">
					<div class="dialog notitle" id="confirmDeletePop" style="display:none;">
						<div class="inner">
							...
							<h5 align="center" class="lineB P25 M0">{$smarty.const.CONFIRM_DELETE_CLIENT_ENTITY}</h5>
